    ...
    if request.method == "POST":
        ...
        if not nome or not email:
            return render_template(
                ...
                erro="Preencha nome e e-mail novamente.",
                nome=nome,
                email=email,
                telefone=telefone,
            )
...
def editar(aluno_id):
    ...
    if request.method == "POST":
        ...
        email_duplicado = Aluno.query.filter(Aluno.email == email, Aluno.id != aluno_id).first()
        if email_duplicado:
            return render_template(
                "formulario.html",
                titulo="Editar aluno", # Corrigido o título que estava "Cadastrar"
                erro="Já existe outro aluno cadastrado com este e-mail.",
                nome=nome,
                email=email,
                telefone=telefone,
                aluno_id=aluno.id, # Garante que o ID continue no form se der erro
            )
